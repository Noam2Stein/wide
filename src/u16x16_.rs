...
impl_simd! {
  ...
  pub fn shuffle(self, indices: u16x16) -> Self {
    pick! {
      if #[cfg(all(target_feature = "avx512bw", target_feature = "avx512vl"))] {
        ...
        Self { avx: unsafe { m256i(_mm256_permutexvar_epi16(indices.avx.0, self.avx.0)) } }
      } else if #[cfg(any(
  ...
  #[inline]
  pub fn zeroing_shuffle(self, indices: u16x16) -> Self {
    pick! {
      if #[cfg(any(
        target_feature = "ssse3",
        all(target_arch = "aarch64", target_feature = "neon"),
  ...
  #[inline]
  pub fn wrapping_shuffle(self, indices: u16x16) -> Self {
    pick! {
      if #[cfg(all(target_feature = "avx512bw", target_feature = "avx512vl"))] {
    ...
    if const { INPUTS == 0 } {
      ...
      pick! {
        if #[cfg(all(target_feature = "avx512bw", target_feature = "avx512vl"))] {
          ...
          u16x16 {
            avx: unsafe {
              m256i(_mm256_permutex2var_epi16(self[0].avx.0, indices.avx.0, self[1].avx.0))
            },
          }
        } else {
          ...
          let self_bytes = unsafe {
            core::mem::transmute_copy::<[u16x16; INPUTS], [u8x32; INPUTS]>(&self)
          };
  ...
  fn zeroing_shuffle(self, indices: Self::Indices) -> Self::Output {
    if const { INPUTS == 0 } {
      ...
      self.shuffle(indices) & indices.simd_lt(const { 16 * INPUTS as u16 })
    }
  }
  ...
  fn wrapping_shuffle(self, indices: Self::Indices) -> Self::Output {
    if const { INPUTS == 0 } {
      ...
      self.shuffle(indices & const { 16 * INPUTS as u16 - 1 })
    }
  }
