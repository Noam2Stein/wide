...
impl_simd! {
  ...
  pub fn shuffle(self, indices: u32x4) -> Self {
    pick! {
      if #[cfg(target_feature = "avx")] {
        ...
        cast::<u8x16, u32x4>(self_bytes.shuffle(byte_indices))
      } else {
    ...
    if const { INPUTS == 0 } {
      ...
      pick! {
        if #[cfg(all(target_feature = "avx512f", target_feature = "avx512vl"))] {
          #[cfg(target_arch = "x86")]
          use core::arch::x86::_mm_permutex2var_epi32;
          #[cfg(target_arch = "x86_64")]
          use core::arch::x86_64::_mm_permutex2var_epi32;
          // TODO(safe_arch): add `_mm_permutex2var_epi32`.
          u32x4 {
            sse: unsafe {
              m128i(_mm_permutex2var_epi32(self[0].sse.0, indices.sse.0, self[1].sse.0))
            },
          }
        } else {
          ...
          let self_bytes = unsafe {
            core::mem::transmute_copy::<[u32x4; INPUTS], [u8x16; INPUTS]>(&self)
          };
  ...
  fn zeroing_shuffle(self, indices: Self::Indices) -> Self::Output {
    if const { INPUTS == 0 } {
      ...
      self.shuffle(indices) & indices.simd_lt(const { 4 * INPUTS as u32 })
    }
  }
