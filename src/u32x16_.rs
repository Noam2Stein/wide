...
impl_simd! {
  ...
  fn shuffle(self, indices: Self::Indices) -> Self::Output {
    ...
    // have an implementation similar to `u32x8` (with `to_byte_indices` and
    ...
    if const { INPUTS == 0 } {
      ...
      pick! {
        if #[cfg(all(target_feature = "avx512f"))] {
          ...
          u32x16 {
            avx512: unsafe {
              m512i(_mm512_permutex2var_epi32(self[0].avx512.0, indices.avx512.0, self[1].avx512.0))
            },
          }
        } else {
          self[0].zeroing_shuffle(indices) | self[0].zeroing_shuffle(indices - 16)
        }
      }
    } else {
      (0..INPUTS).map(|input| self[input].zeroing_shuffle(indices - input as u32 * 16))
        .reduce(BitOr::bitor)
        .unwrap()
    }
  }
  ...
  fn zeroing_shuffle(self, indices: Self::Indices) -> Self::Output {
    if const { INPUTS == 0 } {
      ...
      self.shuffle(indices) & indices.simd_lt(const { 16 * INPUTS as u32 })
    } else {
  ...
  fn wrapping_shuffle(self, indices: Self::Indices) -> Self::Output {
    if const { INPUTS == 0 } {
      ...
      self.shuffle(indices & const { 16 * INPUTS as u32 - 1 })
    }
  }
