...
impl_simd! {
  ...
  fn shuffle(self, indices: Self::Indices) -> Self::Output {
    ...
    // not implement `Pod`.
    cast(unsafe {
      core::mem::transmute_copy::<[i16x8; INPUTS], [u16x8; INPUTS]>(&self).shuffle(indices)
    })
  }
  ...
  fn zeroing_shuffle(self, indices: Self::Indices) -> Self::Output {
    ...
    // not implement `Pod`.
    cast(unsafe {
      core::mem::transmute_copy::<[i16x8; INPUTS], [u16x8; INPUTS]>(&self).zeroing_shuffle(indices)
    })
  }
  ...
  fn wrapping_shuffle(self, indices: Self::Indices) -> Self::Output {
    ...
    // not implement `Pod`.
    cast(unsafe {
      core::mem::transmute_copy::<[i16x8; INPUTS], [u16x8; INPUTS]>(&self).wrapping_shuffle(indices)
    })
  }
